...
import { createDynamicEntityClass, EntityClassType } from "@shared/factory";
...
/**
 * Simple dynamic module with basic entity support
 * This module creates entities without any decorators, just basic TypeORM entity structure
 */
@Global()
@Module({})
export class CrudConfigSimpleDynamicModule implements OnModuleInit {
	constructor(@Inject(getDataSourceToken()) private readonly dataSource: DataSource) {}

	/**
	 * Get the dynamic entities (for use in TypeORM configuration)
	 */
	public static getEntities(): Array<EntityClassType> {
		if (!globalSectionEntity || !globalDataEntity) {
			throw new Error("CrudConfigSimpleDynamicModule must be registered before accessing entities");
		}
	...
	/**
	 * Registers the module with simple dynamic entity support
	 * @param options
	 */
	public static register(options: ICrudConfigProperties): DynamicModule {
		const prefix = options.entityOptions?.tablePrefix || "";

		// Create simple entities without decorators
		const sectionEntity = createDynamicEntityClass({
			name: 'ConfigSection',
			tableName: prefix + (options.entityOptions?.configSection?.tableName || "config_section"),
			columns: {
				name: {
					type: 'varchar',
					length: options.entityOptions?.configSection?.maxNameLength || 128,
					nullable: false,
				},
				description: {
					type: 'varchar',
					length: options.entityOptions?.configSection?.maxDescriptionLength || 512,
					nullable: true,
				},
				createdAt: {
					type: 'timestamp',
		...
		const dataEntity = createDynamicEntityClass({
			name: 'ConfigData',
			tableName: prefix + (options.entityOptions?.configData?.tableName || "config_data"),
			columns: {
				name: {
					type: 'varchar',
					length: options.entityOptions?.configData?.maxNameLength || 128,
					nullable: false,
				},
				value: {
					type: 'varchar',
					length: options.entityOptions?.configData?.maxValueLength || 8192,
					nullable: false,
				},
				description: {
					type: 'varchar',
					length: options.entityOptions?.configData?.maxDescriptionLength || 512,
					nullable: true,
				},
				environment: {
					type: 'varchar',
					length: options.entityOptions?.configData?.maxEnvironmentLength || 64,
					nullable: false,
				},
				isEncrypted: {
					type: 'boolean',
			...
			relations: {
				section: {
					type: 'ManyToOne',
					decorator: { target: sectionEntity },
					eager: false,
					nullable: false,
					onDelete: 'CASCADE',
				},
		...
		return {
			controllers: [], // Controllers not included in this module variant
			...
			imports,
			module: CrudConfigSimpleDynamicModule,
	...
	async onModuleInit() {
		...
		console.log(
			"CrudConfigSimpleDynamicModule initialized with entities:",
