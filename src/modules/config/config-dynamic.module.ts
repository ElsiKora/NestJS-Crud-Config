...
/**
 * Dynamic module without ApiPropertyDescribe support
 * This module creates entities without API documentation decorators
 */
@Global()
@Module({})
export class CrudConfigDynamicModule implements OnModuleInit {
	...
	public static getEntities(options?: ICrudConfigProperties): Array<EntityClassType> {
		if (!globalSectionEntity || !globalDataEntity) {
			if (!options) {
				throw new Error("CrudConfigDynamicModule must be registered before accessing entities, or provide options to initialize them");
			}
			// Initialize entities if not already done
			const prefix = options.entityOptions?.tablePrefix || "";

			// Create entities without API decorator support
	...
	/**
	 * Registers the module with dynamic entity support without ApiPropertyDescribe
	 * @param options
	 */
	public static register(options: ICrudConfigProperties): DynamicModule {
		...
		return {
			...
			imports,
			module: CrudConfigDynamicModule,
	...
	async onModuleInit() {
		...
		console.log(
			"CrudConfigDynamicModule initialized with entities:",
